...
def receive_data():
    ...
    try:
        sky =sky_df.loc[sky_df['category'] == 'SKY', 'fcstValue'].map(sky_dict).values[0]
    ...
    muse_data = data.get('museums', [])
    lib_data = data.get('libraries', [])
    park_data = data.get('parks', [])

    # 데이터프레임 생성 및 전처리
    muse = pd.DataFrame(muse_data)
    print("Museum DataFrame:", muse_df)  # 데이터프레임 확인
    ...
    print("Library DataFrame:", lib_df)  # 데이터프레임 확인
    ...
    print("Park DataFrame:", park_df)  # 데이터프레임 확인
    ...
    if (sky in ['맑음', '흐림']) and (rain == '강수없음') and (15 <= float(temp) <= 29):
        ...
        print("Inside the elif condition")
        # 가장 가까운 도서관 찾기
        min_distance = float('inf')
